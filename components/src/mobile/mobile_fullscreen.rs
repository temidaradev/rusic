...
pub fn MobileFullscreen(
    ...
    rsx! {
        div {
            class: "fixed inset-0 z-50 flex flex-col text-white select-none overflow-y-auto",
            style: "background-color: var(--color-black);",

            div {
                class: "flex flex-col items-center justify-start p-6 w-full mx-auto shrink-0",
                style: "max-width: 520px;",

                div {
                    class: "w-full flex justify-between items-center mb-6 mt-4",
                    button {
                        class: "text-white/50 hover:text-white transition-colors p-2 -ml-2",
                        onclick: move |_| is_fullscreen.set(false),
                        i { class: "fa-solid fa-chevron-down text-xl" }
                    }
                    span { class: "text-xs font-medium text-white/50 tracking-widest", "NOW PLAYING" }
                    div { class: "w-8" }
                }

                div {
                    class: "rounded-lg overflow-hidden shadow-2xl mb-8 w-full max-w-[320px] aspect-square",
                ...
                div {
                    class: "flex items-center justify-between mb-8 w-full px-4",
                    button {
                        class: if *ctrl.shuffle.read() { "text-white hover:text-white/80 transition-colors flex-shrink-0" } else { "text-white/50 hover:text-white transition-colors flex-shrink-0" },
                    ...
                    button {
                        class: if *ctrl.loop_mode.read() == hooks::use_player_controller::LoopMode::None { "text-white/50 hover:text-white transition-colors flex-shrink-0 relative flex justify-center" } else { "text-white hover:text-white/80 transition-colors flex-shrink-0 relative flex justify-center" },
                        onclick: move |_| ctrl.toggle_loop(),
                        i { class: "fa-solid fa-repeat" }
                        if *ctrl.loop_mode.read() == hooks::use_player_controller::LoopMode::Track {
                            span {
                                style: "position: absolute; font-size: 10px; font-weight: bold; top: 100%; left: 50%; transform: translateX(-50%); line-height: 1;",
                                "1"
                            }
                        }
                    }
            ...
            div {
                class: "flex flex-col w-full mx-auto pb-10",
                style: "max-width: 520px; min-height: 400px;",
                ...
                div {
                    class: "flex-1 px-4 py-2 space-y-1 mb-8",
